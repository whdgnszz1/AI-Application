...
def main():
    ...
    # 층 정규화 적용
    norm = nn.LayerNorm(embedding_dim).to(device)
    norm_x = norm(input_embeddings)
    print("norm_x.shape:", norm_x.shape)  # torch.Size([1, 5, 16])
    print("norm_x.mean(dim=-1).data:", norm_x.mean(dim=-1).data)
    print("norm_x.std(dim=-1).data:", norm_x.std(dim=-1).data)
    ...
    # 멀티헤드 어텐션을 통해 순전파를 수행 (정규화된 임베딩 사용)
    after_attention_embeddings = multihead_attention(norm_x, norm_x, norm_x)
